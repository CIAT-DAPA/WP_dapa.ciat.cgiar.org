...
function postimage($postId) {
  ...
  if (has_post_thumbnail($postId)) {
    ...
    $img = wp_get_attachment_image_src(get_post_thumbnail_id($postId), 'large');
    /*$img = wp_get_attachment_image_src(get_post_thumbnail_id($postId), 'medium');*/
    return $img[0];
  } else {
...
//      echo " / ";
      $cat = str_replace('<ul class="post-categories">', "", the_category());
      //echo $cat;
      //if (is_single()) {
        //the_title();
      //  single_cat_title();
...
if (!function_exists('theme_get_metadata_icons')) {
  ...
  function theme_get_metadata_icons($icons = '', $class = '') {
    ...
    for ($i = 0; $i < count($icons); $i++) {
      ...
      switch ($icon) {
        case 'date':
          $result[] = sprintf(__('%2$s', THEME_NS), 'date', sprintf('<span class="entry-date" title="%1$s">%2$s</span>', esc_attr(get_the_time()), get_the_date()
                          )
                  ) . '</span>';
          break;
        case 'author':
          $result[] = sprintf(__('<span class="%1$s">by</span> %2$s', THEME_NS), 'author', sprintf('<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a></span>', get_author_posts_url(get_the_author_meta('ID')), sprintf(esc_attr(__('View all posts by %s', THEME_NS)), get_the_author()), get_the_author()
                          )
                  ) . '</span>';
          break;
        case 'category':
...
//add_action('init', 'add_custom_feed');
   ...
   * @since 2.1.0
   */
  function do_feed_ciat_rss2_es() {
    load_template(TEMPLATEPATH . '/feed-rss2-es.php');
  }
   ...
   * @since 2.1.0
   */
  function do_feed_ciat_rss2_en() {
    load_template(TEMPLATEPATH . '/feed-rss2-en.php');
  }
  ...
  add_action('do_feed_ciat_rss2_es', 'do_feed_ciat_rss2_es', 10, 1);

  add_action('do_feed_ciat_rss2_en', 'do_feed_ciat_rss2_en', 10, 1);
