<?php
/*
 * Plugin Name: Themedy - 300x250 Ad Unit
 ...
 */
add_action( 'widgets_init', 'themedy_ad300_widgets' );

/*
 * Register widget.
 */
function themedy_ad300_widgets() {
	register_widget( 'Themedy_Ad300_Widget' );
}

/*
 * Widget class.
 */
class themedy_ad300_widget extends WP_Widget {

	/* ---------------------------- */
	/* -------- Widget setup -------- */
	/* ---------------------------- */
	
	function Themedy_Ad300_Widget() {
	
		/* Widget settings */
		$widget_ops = array( 'classname' => 'themedy_ad300_widget', 'description' => __('A widget that allows the display and configuration of of a single 300x250 Banner.', 'framework') );

		/* Widget control settings */
		//$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'themedy_ad300_widget' );

		/* Create the widget */
		$this->WP_Widget( 'themedy_ad300_widget', __('Themedy - 300x250 Ad', 'framework'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		$defaults = array(
		'title' => '',
		'ad' => get_bloginfo('stylesheet_directory')."/images/ad-300x250.png",
