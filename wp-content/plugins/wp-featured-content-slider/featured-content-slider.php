...
$c_slider_options_page = site_url( '/wp-admin/admin.php?page=wp-featured-content-slider/options.php' );

function c_slider_options_page() {
        
	add_options_page('Featured Content Slider Options', 'Featured Content Slider', 'manage_options', 'wp-featured-content-slider/options.php');
...
function c_slider_add_scripts() {
        ...
        if ( !is_admin() ) {
                ...
                wp_register_script('jquery.cycle', get_bloginfo('url') . '/wp-content/plugins/wp-featured-content-slider/scripts/jquery.cycle.all.3.0.3.js', array('jquery'), '1.3' );
...
function c_slider_cut_text($text, $chars, $points = "...") {
	...
	if($length <= $chars) {
		...
		/* return $content; */
            return mb_substr($content, 0, $chars);
...
?>
		...
		<table class="form-table">
			<tr>
				<th><label for="feat_slider">The blog administrator decides when to activate this option.</label></th>
				<td><input type="checkbox" name="feat_slider" value="1" style="display:none;" <?php if($feat_slider == 1) { echo "checked='checked'";} ?></td>
			</tr>
		</table>
	</div>
