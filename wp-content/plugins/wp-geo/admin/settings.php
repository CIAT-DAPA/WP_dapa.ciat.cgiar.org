...
class WPGeo_Settings {
	...
	function default_map_location_field() {
		...
		echo '<tr><th><label for="default_map_latitude">' . esc_html__( 'Latitude', 'wp-geo' ) . '</label></th><td><input name="wp_geo_options[default_map_latitude]" type="text" id="default_map_latitude" value="' . esc_attr( $options['default_map_latitude'] ) . '" size="25" /></td></tr>';
		echo '<tr><th><label for="default_map_longitude">' . esc_html__( 'Longitude', 'wp-geo' ) . '</label></th><td><input name="wp_geo_options[default_map_longitude]" type="text" id="default_map_longitude" value="' . esc_attr( $options['default_map_longitude'] ) . '" size="25" /></td></tr>';
		echo '</table>';
	}
	...
	function default_map_width_field() {
		$options = get_option( 'wp_geo_options' );
		echo '<input name="wp_geo_options[default_map_width]" type="text" id="default_map_width" value="' . esc_attr( $options['default_map_width'] ) . '" size="10" />';
	}
	...
	function default_map_height_field() {
		$options = get_option( 'wp_geo_options' );
		echo '<input name="wp_geo_options[default_map_height]" type="text" id="default_map_height" value="' . esc_attr( $options['default_map_height'] ) . '" size="10" />';
	}
	...
	function default_map_controls_field() {
		...
		echo '<p style="margin:1em 0 0 0;"><strong>' . esc_html__( 'Map Type Controls', 'wp-geo' ) . '</strong></p>';
		echo '<p style="margin:0;">' . esc_html__( 'You must select at least 2 map types for the control to show.', 'wp-geo' ) . '</p>';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_map_type_normal]', 'Y', $options['show_map_type_normal'], false, 'show_map_type_normal' ) . ' ' . esc_html__( 'Normal map', 'wp-geo' ) . '</label><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_map_type_satellite]', 'Y', $options['show_map_type_satellite'], false, 'show_map_type_satellite' ) . ' ' . esc_html__( 'Satellite (photographic map)', 'wp-geo' ) . '</label><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_map_type_hybrid]', 'Y', $options['show_map_type_hybrid'], false, 'show_map_type_hybrid' ) . ' ' . esc_html__( 'Hybrid (photographic map with normal features)', 'wp-geo' ) . '</label><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_map_type_physical]', 'Y', $options['show_map_type_physical'], false, 'show_map_type_physical' ) . ' ' . esc_html__( 'Physical (terrain map)', 'wp-geo' ) . '</label><br />';
		echo '<p style="margin:1em 0 0 0;"><strong>' . esc_html__( 'Other Controls', 'wp-geo' ) . '</strong></p>';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_map_scale]', 'Y', $options['show_map_scale'], false, 'show_map_scale' ) . ' ' . esc_html__( 'Show map scale', 'wp-geo' ) . '</label><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_map_overview]', 'Y', $options['show_map_overview'], false, 'show_map_overview' ) . ' ' . esc_html__( 'Show collapsible overview map (in the corner of the map)', 'wp-geo' ) . '</label><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_streetview_control]', 'Y', $options['show_streetview_control'], false, 'show_streetview_control' ) . ' ' . esc_html__( 'Show Street View Control', 'wp-geo' ) . '</label>';
	}
	...
	function default_post_options_field() {
		global $wpgeo;
		$options = get_option( 'wp_geo_options' );
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[save_post_zoom]', 'Y', $options['save_post_zoom'], false, 'save_post_zoom' ) . ' ' . esc_html__( 'Save custom map zoom for this post', 'wp-geo' ) . '</label><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[save_post_map_type]', 'Y', $options['save_post_map_type'], false, 'save_post_map_type' ) . ' ' . esc_html__( 'Save custom map type for this post', 'wp-geo' ) . '</label><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[save_post_centre_point]', 'Y', $options['save_post_centre_point'], false, 'save_post_centre_point' ) . ' ' . esc_html__( 'Save map centre point for this post', 'wp-geo' ) . '</label>';
	...
	function show_polylines_field() {
		global $wpgeo;
		$options = get_option( 'wp_geo_options' );
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_polylines]', 'Y', $options['show_polylines'], false, 'show_polylines' ) . ' ' . esc_html__( 'Show polylines (to connect multiple points on a single map)', 'wp-geo' ) . '</label>';
	...
	function polyline_colour_field() {
		$options = get_option( 'wp_geo_options' );
		echo '<input name="wp_geo_options[polyline_colour]" type="text" id="polyline_colour" value="' . esc_attr( $options['polyline_colour'] ) . '" size="7" />';
	}
	...
	function show_maps_on_field() {
		...
		echo '<strong>' . esc_html__( 'Single Pages', 'wp-geo' ) . '</strong><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_maps_on_pages]', 'Y', $options['show_maps_on_pages'], false, 'show_maps_on_pages' ) . ' ' . esc_html__( 'Pages', 'wp-geo' ) . '</label><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_maps_on_posts]', 'Y', $options['show_maps_on_posts'], false, 'show_maps_on_posts' ) . ' ' . esc_html__( 'Posts', 'wp-geo' ) . '</label><br />';
		...
		foreach ( $post_types as $post_type ) {
			if ( post_type_supports( $post_type->query_var, 'wpgeo' )) {
				$custom_post_type_checkboxes .= '<label>' . wpgeo_checkbox( 'wp_geo_options[show_maps_on_customposttypes][' . $post_type->query_var . ']', 'Y', 'Y', true ) . ' ' . esc_html( $post_type->label ) . '</label><br />';
				...
				$custom_post_type_checkboxes .= '<label>' . wpgeo_checkbox( 'wp_geo_options[show_maps_on_customposttypes][' . $post_type->query_var . ']', 'Y', $custom_post_type_checkbox_value, false ) . ' ' . esc_html( $post_type->label ) . '</label><br />';
			}
		}
		...
		echo '<strong>' . esc_html__( 'Posts on Taxonomy Pages', 'wp-geo' ) . '</strong><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_maps_in_categoryarchives]', 'Y', $options['show_maps_in_categoryarchives'], false, 'show_maps_in_categoryarchives' ) . ' ' . esc_html__( 'Categories', 'wp-geo' ) . '</label><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_maps_in_tagarchives]', 'Y', $options['show_maps_in_tagarchives'], false, 'show_maps_in_tagarchives' ) . ' ' . esc_html__( 'Tags', 'wp-geo' ) . '</label><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_maps_in_taxarchives]', 'Y', $options['show_maps_in_taxarchives'], false, 'show_maps_in_taxarchives' ) . ' ' . esc_html__( 'All Taxonomies', 'wp-geo' ) . '</label><br />';

		echo '<strong>' . esc_html__( 'Posts on Other Pages', 'wp-geo' ) . '</strong><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_maps_on_home]', 'Y', $options['show_maps_on_home'], false, 'show_maps_on_home' ) . ' ' . esc_html__( 'Posts Archive', 'wp-geo' ) . '</label><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_maps_in_datearchives]', 'Y', $options['show_maps_in_datearchives'], false, 'show_maps_in_datearchives' ) . ' ' . esc_html__( 'Date Archives', 'wp-geo' ) . '</label><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_maps_in_authorarchives]', 'Y', $options['show_maps_in_authorarchives'], false, 'show_maps_in_authorarchives' ) . ' ' . esc_html__( 'Author Archives', 'wp-geo' ) . '</label><br />';
		echo '<label>' . wpgeo_checkbox( 'wp_geo_options[show_maps_in_searchresults]', 'Y', $options['show_maps_in_searchresults'], false, 'show_maps_in_searchresults' ) . ' ' . esc_html__( 'Search Results', 'wp-geo' ) . '</label><br />';
	}
	...
	function wpgeo_filter_plugin_action_links( $links, $file ) {
		if ( $file == 'wp-geo/wp-geo.php' ) {
			$settings_link = '<a href="' . admin_url( 'options-general.php?page=wp-geo' ) . '">' . esc_html__( 'Settings', 'wp-geo' ) . '</a>';
